...
BEGIN
	...
	daynum := EXTRACT(DOW FROM Tdate)+1;  -- Get the day of the week as an integer
        ...
        WHILE j <= slots LOOP
            ...
            INSERT INTO ms_appointment_slot_calendar (slot_doct_id, slot_date, slot_from, slot_to)
            VALUES (doc_id, Tdate, ftime, ttime);
...
---------------------------------------------------------
--- proceudure for all doctors
CREATE OR REPLACE PROCEDURE generate_all_doctor_slots()
LANGUAGE plpgsql
AS $$
DECLARE
   	doctId int:=0;
   	doctor_cursor CURSOR FOR SELECT doc_id FROM arogya_doctors;
BEGIN 
    OPEN doctor_cursor;
    LOOP
		FETCH doctor_cursor INTO doctId;
		EXIT WHEN NOT FOUND;
		CALL generate_doctor_slots(doctId);
    END LOOP; 
    CLOSE doctor_cursor;
END;
$$;
